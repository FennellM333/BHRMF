...
filenamelist =  '/mnt/data0/jillian/h568/density5000/files.list'
files = np.loadtxt(filenamelist, dtype = np.str)

nfiles = len(files)

for i in range(nfiles):
    s = pynbody.load(files[i])
    h = s.halos()
    s.physical_units()
...
with open('foundden5000.txt' , 'w') as f:
